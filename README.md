...
> **왜 브라우저 자동화인가?** Tistory Open API가 2024년에 종료되어, 글을 자동으로 발행하는 공식 경로가 없기 때문에 차선책으로 발행 단계는 사람이 에디터를 쓰는 것을 흉내 내는 Playwright 우회 방식으로 동작합니다.
...
`config.example.py`를 복사해 `config.py`를 만들고 값을 채웁니다. (`config.py`는 `.gitignore`에 포함되어 커밋되지 않고 **본인 로컬에만 저장**되어 **개인정보가 보호**됩니다.)
...
**이미 올라갔지만, 이슈가 발생해서 다시 발행하고 싶은 글이 있다면**, published.json에서 해당 글의 제목을 삭제한 후 다시 발행(`python 4_publish_tistory.py`) 해주세요.
...
에디터에 값은 들어갔지만 Tistory의 저장 필드에 동기화되지 않은 경우입니다. 해당 프로젝트에서는 프로그래밍 주입 대신 실제 키보드 입력으로 본문을 채워 이를 방지하도록 수정했습니다. 그래도 발생하면 github issues에 등록해주세요!
...
이미지 저장소가 <b>private</b>이면 jsDelivr가 서빙하지 못합니다. 저장소를 public으로 바꿔주세요. 방금 public으로 전환했다면 CDN 캐시가 갱신될 때까지 몇 분 기다린 후 다시 시도해주세요!
...
Tistory 에디터 DOM이 바뀌면 셀렉터가 어긋날 수 있습니다. <code>python 4_publish_tistory.py --limit 1</code> 로 실행하면 브라우저가 열린 채 멈추므로, Playwright Inspector의 <b>Pick locator</b>로 실제 셀렉터를 확인해 수정하거나 github issues로 남겨주시면 확인 후 수정하겠습니다!
...
- 공식 자동화 프로그램이 아니고 우회해서 사용한 방식이기 때문에, 이후에 Tistory 에디터 UI가 변경되면 셀렉터가 깨질 수 있습니다.
